...
#define PIN_PAN_HALL Z_OPTO
#define PIN_TILT_HALL X_OPTO
...
AccelStepper panStepper = AccelStepper(AccelStepper::DRIVER, Z_STEP, Z_DIR);
AccelStepper tiltStepper = AccelStepper(AccelStepper::DRIVER, X_STEP, X_DIR);
...
float pan_max_speed = 100; //degrees/second. Deze kan dus aangepast worden naar hoelang de beweging mag duren. De eis was om binnen 4 sec 45 graden te halen, en dat haal je hier wel mee.
float tilt_max_speed = 100; //degrees/second.
...
void handleCommand(String command) {
  if (command.startsWith("p_target=")) {
    ...
    Serial.println(panStepsToDegrees(target));
